...
if (not is_remove_yaw_set_)
    ...
    if ( base_pub_counter%4 == 0)
    {
        ...
        imu_orientation_raw_[2] = static_cast<double>(aliengo_state_.imu.quaternion[2]);  // y
        imu_orientation_raw_[3] = static_cast<double>(aliengo_state_.imu.quaternion[3]);  // z
        ...
        imu_euler_raw_[0] = static_cast<double>(aliengo_state_.imu.rpy[0]);  // R
        imu_euler_raw_[1] = static_cast<double>(aliengo_state_.imu.rpy[1]);  // P
        imu_euler_raw_[2] = static_cast<double>(aliengo_state_.imu.rpy[2]);  // Y
...
void AliengoRobotHw::write()
{
    ...
    for (unsigned int jj = 0; jj < n_dof_; ++jj)
    {
      aliengo_lowcmd_.motorCmd[aliengo_motor_idxs_[jj]].mode = 0x0A;  // motor switch to servo (PMSM) mode
      aliengo_lowcmd_.motorCmd[aliengo_motor_idxs_[jj]].tau = static_cast<float>(joint_effort_command_[jj]  );
      //these to be sure to have pure torque control mode
      aliengo_lowcmd_.motorCmd[aliengo_motor_idxs_[jj]].q = unitree::PosStopF; 
      aliengo_lowcmd_.motorCmd[aliengo_motor_idxs_[jj]].Kp = 0;
      aliengo_lowcmd_.motorCmd[aliengo_motor_idxs_[jj]].dq = unitree::VelStopF; 
      aliengo_lowcmd_.motorCmd[aliengo_motor_idxs_[jj]].Kd = 0;
    }
    // aliengo_lowcmd_.head[0] = 0xFE;
		// aliengo_lowcmd_.head[1] = 0xEF;
		aliengo_lowcmd_.levelFlag = unitree::LOWLEVEL;
